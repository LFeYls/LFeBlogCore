...
namespace LFeBlog.Infrastructure.Extensions
{
    public static class QueryableExtensions
    {
        ...
        public static IQueryable<object> ToDynamicQueryable<TSource>(this IQueryable<TSource> source, string fields,
            Dictionary<string, List<MappedProperty>> mappingDictionary)
        {
            if (source==null)
            {
                throw new ArgumentNullException(nameof(source));
            ...
            foreach (var field in fieldsAfterSplit)
            {
                ...
                var key = mappingDictionary.Keys.SingleOrDefault(c =>
                    string.CompareOrdinal(c.ToLower(), propertyName.ToLower()) == 0);
